...
class CalculateGraph : CommandLineRunner, Serializable {
    ...
    override fun run(vararg args: String) {
        ...
        val buildFirst = SourceBuilder
                .timestampedStream<SubGraphData>("some-data-source-first", { SubGraphData("", "", mapOf()) })
        ...
        val buildSecond = SourceBuilder
                .timestampedStream<SubGraphData>("some-data-source-second", { SubGraphData("", "", mapOf()) })
                .fillBufferFn(BiConsumerEx<SubGraphData, TimestampedSourceBuffer<SubGraphData>> { obj, buf ->
                    if (secondGraph && currentTime + delta < System.currentTimeMillis()) {
                        buf.add(getSecondData(), System.currentTimeMillis())
                        secondGraph = false
        ...
        val withNativeTimestampsFirst = pipelineFirst.readFrom<SubGraphData>(buildFirst)
                .withNativeTimestamps(0)

        val withNativeTimestampsSecond = pipelineFirst.readFrom<SubGraphData>(buildSecond)
                .withNativeTimestamps(0)
...
//        instance.newJob(pipelineSecond, JobConfig().setName(" graph Second"))
    ...
    private fun secondGraph1(withNativeTimestamps: StreamStage<SubGraphData>): StreamStage<ResultCheckData> {
    ...
    private fun firstGraph(withNativeTimestamps: StreamStage<SubGraphData>): StreamStage<ResultCheckData> {
        val chkFirst = withNativeTimestamps
    ...
    private fun notNullCeck(chkFirst: StreamStage<SubGraphDataEndCheck>, name: String, field: String, check: String): StreamStage<ResultCheckData> {
        return chkFirst
                .setName(name)
                .map {
                    if (it.subGraphData.data[field] == null) {
                        ResultCheckData(it.subGraphData.bsnKey, check, "$field is null")
                    } else null
                }
//                .filter { it != null }
    }
    ...
    private fun getSecondData(): SubGraphData {
        return SubGraphData(BSN_KEY, SECOND_GRAPH, mapOf("s1" to 12, "s2" to "as"))
    }
